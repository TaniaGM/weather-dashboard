...
var searchHistory = $("#search-his");
var weatherCol = $("#weather-col");

var apiKey = "dd476d396302cc89393382ed7c0f93e2";
var currentWeatherUrl;
var forecastUrl;
var storedSearch = [];


var tempStoredSearch = localStorage.getItem("storedSearch");
if (tempStoredSearch != null)
    storedSearch = tempStoredSearch.split(",");
...
function populateWeatherForecast() {
    ...
    $.ajax({
        ...
        for (var i = 4; i < response.list.length; i += 8) {
            ...
            fiveDayArray.push(tempForecastObj);
        }

        for (var i = 0; i < fiveDayArray.length; i++) {
            fiveDayArray[i].date = formatDates(fiveDayArray[i].date);
        }
        ...
        for (var i = 0; i < fiveDayArray.length; i++) {
            var forecastCard = $('<div class="col-lg-2 col-sm-3 mb-1"><span class="badge badge-primary"><h5>' + 
            fiveDayArray[i].date + '</h5>' +
                '<p><img class="w-100" src="http://openweathermap.org/img/wn/' + 
                fiveDayArray[i].weatherIcon + 
                '@2x.png"></p>' +
                '<p>Temp: ' + 
                fiveDayArray[i].temperature + 
                '°F</p>' +
                '<p>Humidity: ' + 
                fiveDayArray[i].humidity + 
                '%</p>' +
                '<span></div>');
...
function renderStoredSearch() {
    ...
    if ($("#search-bar").val() != "") {
        if (storedSearch.indexOf($("#search-bar").val()) != -1) {
            storedSearch.splice(storedSearch.indexOf($("#search-bar").val()), 1)
        }
        storedSearch.unshift($("#search-bar").val());
    }
